...
import com.weibo.login.NetStateManager;
import com.weibo.login.Utility;
import com.weibo.login.WeiboException;
import com.wejoy.net.HTTPParameters;
import com.wejoy.net.NetUtil;
...
public class WeJoyHTTPManager {
	 ...
	 */
	public static String openUrl(String url, String method, HTTPParameters params, String file) throws WeiboException {
		String result = "";
		...
		try {
			...
			if (method.equals(HTTPMETHOD_GET)) {
				...
				if (!TextUtils.isEmpty(file)) {
					...
					ImageUtils.revisionPostImageSize(  file);
	...
	private static HttpClient getNewHttpClient() {
		if(httpClient == null) {
			synchronized(WeJoyHTTPManager.class) {
				if(httpClient == null) {
					try {
...
//						}
	...
	private static String readHttpResponse(HttpResponse response) {
		String result = new String(readHttpResponse0(response));
		return result;
	}
	
	private static byte[] readHttpResponse0(HttpResponse response) {
	...
	public static byte[] getImage(String url) throws WeiboException {
		try {
			HttpClient client = getNewHttpClient();
			client.getParams().setParameter(ConnRoutePNames.DEFAULT_PROXY, NetStateManager.getAPN());
			HttpGet get = new HttpGet(url);
			...
			byte[] data = readHttpResponse0(response);
			return data;
		} 
		catch (IOException e) {
			throw new WeiboException(e);
